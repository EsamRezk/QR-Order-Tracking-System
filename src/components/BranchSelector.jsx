...
export default function BranchSelector({ value, onChange, includeAll = false }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <span className="text-sm font-medium text-[#2E2D2C]">
        ...
        >
          <path fill="#5830C5" d="M6 8L1 3h10z" />
        </svg>
      </div>

      {/* Dropdown */}
      <div
        className={`absolute top-full right-0 mt-2 min-w-[200px] bg-[#ffffff] border border-[#e5e7eb] rounded-xl shadow-xl z-50 overflow-hidden transition-all duration-200 origin-top
            ...
            return (
              <div
                ...
                className={`px-4 py-2.5 cursor-pointer transition-colors text-sm font-medium rounded-lg ${
                  isSelected
                    ? 'bg-[#5830C5] text-white'
                    : 'text-[#2E2D2C] hover:bg-[#f3f4f6]'
                }`}
              >
                {opt.name_ar}
              </div>
