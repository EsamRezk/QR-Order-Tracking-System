...
- **تخطيط الشبكة (الحالي = تصميم أفقي):** الكرت **مستطيل أفقي واطي** (`flex-direction: row` + بادينج خفيف ⇒ ارتفاع ≈ نص المربع السابق) — **اللوجو صغير يمين (ثابت 46px) والرقم كبير شمال بالعرض في سطر واحد** (`.disp-card-order{flex:1}` + `.disp-card-id{white-space:nowrap}` عشان الرقم ما يتقسّمش عمودي)، و**10 طلبات في الصف** (`repeat(10,1fr)`). هيدر مضغوط + ساعة بالأرقام الإنجليزية (`formatClock` بلا AM/PM). لوجو كبة زون الثابت (`AppLogo`) يصغُر/يرتفع على مسار `/display` فقط. **نسخة التصميم الرأسي السابق (12/صف، لوجو يملأ العرض) محفوظة في scratchpad (`DisplayDashboard.design1.css/.jsx`) للرجوع إليها.**
