...
- **URL:** `/analytics`
- **بحث بالطلب:** حقل بحث برقم الطلب (ilike) — يبحث في كل الداتا بيز بغض النظر عن الفلاتر، يعرض النتائج في جدول منفصل (نفس صف `OrderRow`)
- **الجلب:** كل الطلبات (بكل الحالات) ضمن الفترة/الفرع، `.limit(1000)`، مرتّبة `created_at desc`
- **فلاتر:** فرع + فترة زمنية (اليوم / 7 / 30) **تقود الجلب**؛ والحالة + التطبيق + الرقم **فلاتر محلية على الجدول** (state `statusFilter`/`appFilter`/`numberFilter` → `filteredOrders` memo). الحالة والتطبيق عبر قائمة منسدلة مخصّصة `Dropdown` (التطبيق يعرض اللوجو)؛ الحالة = الفعّالة فقط (قيد التحضير/جاهز/مكتمل)؛ الرقم يبحث بالرقمين (فوديكس reference + رقم التطبيق)
