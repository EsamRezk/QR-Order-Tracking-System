...
- **DisplayCard (مكوّن داخلي مبسّط):** بادج الحالة العلوي + لوجو التطبيق **كبير بلا صندوق** (يملأ عرض الكرت) + رقم الطلب **فقط** — بلا كلمة "طلب"، بلا زر، بلا نوع الطلب، بلا اسم التطبيق، بلا عدّاد وقت. قيد التحضير = لون التطبيق + بادج أصفر؛ جاهز = أزرق + بادج أزرق.
- **تخطيط الشبكة:** **12 طلب في الصف الواحد** على الشاشات الكبيرة (`repeat(12,1fr)`)؛ هيدر مضغوط قدر الإمكان + ساعة بالأرقام الإنجليزية (`formatClock` بلا AM/PM). لوجو كبة زون الثابت (`AppLogo`) يصغُر ويرتفع تلقائياً على مسار `/display` فقط ليلائم الهيدر القصير.
