...
| التاريخ | الوصف |
|---------|-------|
| 2026-07-12 | **🐛 إصلاح عدم ثبات اللوجو وزر السايدبار في صفحة التحليلات أثناء السكرول.** `.analytics-root` كان يستخدم `height:100vh; overflow-y:auto` كحاوية تمرير داخلية مستقلة عن النافذة، بدل ترك `body`/`html` يتمرر طبيعياً. هذا النمط (تمرير على عنصر منفصل غير `body`) يكسر ثبات عناصر `position:fixed` الشقيقة له في الـDOM (`AppLogo` و`AdminSidebar`'s toggle button، المُعرَّفة في `App.jsx` خارج `.analytics-root`) على بعض المتصفحات — فتتحرك مع المحتوى بدل أن تبقى مثبّتة أعلى الشاشة. **الحل:** استُبدل `height:100vh; overflow-y:auto` بـ `min-height:100vh` في `Analytics.css`، فيعود التمرير طبيعياً لـ`body`/`html` (يطابق كل الصفحات الأخرى في المشروع)، ويبقى ثبات رأس الجدول (`position:sticky`) شغالاً كما هو لأن أسلافه تستخدم `overflow:visible` أصلاً. ✅ lint نظيف + build ناجح. |
