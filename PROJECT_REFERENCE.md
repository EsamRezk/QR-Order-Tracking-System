...
| التاريخ | الوصف |
|---------|-------|
| 2026-07-01 | **☑️ شيك بوكسات فلترة على شاشة العرض (`DisplayDashboard`): الكل / النشطة فقط / الجاهزة فقط + نافذة تأكيد + تجاوب جوال.** حسب الطلب: شريط فلترة (`.dash-filter-bar`) أعلى `dash-main` فيه شريحتان (`.dash-filter-chip` بنمط شيك بوكس): **"النشطة فقط"** و**"الجاهزة فقط"**. **السلوك:** كلاهما مطفأ = **يعرض كل الطلبات** (النشطة + المُسلّمة)؛ تفعيل "النشطة فقط" = يخفي قسم "تم تسليمها"؛ "الجاهزة فقط" = يعرض الجاهزة فقط (يخفي قيد التحضير + المُسلّمة). حالة محلية `filterMode ∈ {all,active,ready}` (غير مخزّنة — عرض فقط، الشريحتان متنافيتان). **التأكيد:** نقر شريحة لتفعيلها يفتح `pendingFilter` → نافذة **`.disp-modal`** (تأكيد/إلغاء) ثم `confirmFilter` يطبّق الفلتر؛ نقر شريحة مفعّلة يطفّيها فوراً (رجوع لـ all بلا تأكيد). العدّاد في الهيدر يبقى للنشطة الفعلية. **CSS:** أنماط `.dash-filter-*` + `.disp-modal*` جديدة في `DisplayDashboard.css` + قواعد جوال داخل `@media(max-width:768px)` (الشرائح `flex:1` متمركزة + عنوان بعرض كامل + تصغير النافذة). **تعديل JSX + CSS فقط، بلا مسّ للـ hooks/DB.** ✅ lint + build نظيف. |
