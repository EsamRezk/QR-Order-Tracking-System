...
- **تخطيط الشبكة (الحالي = تصميم أفقي):** الكرت **مستطيل أفقي** (`flex-direction: row`) — **اللوجو يمين (ثابت 96px) والرقم شمال** (RTL يضع أول عنصر يميناً)، بادينج/مسافات خفيفة جداً + فجوة صغيرة بين اللوجو والرقم فقط، و**10 طلبات في الصف** (`repeat(10,1fr)`). هيدر مضغوط + ساعة بالأرقام الإنجليزية (`formatClock` بلا AM/PM). لوجو كبة زون الثابت (`AppLogo`) يصغُر/يرتفع على مسار `/display` فقط. **نسخة التصميم الرأسي السابق (12/صف، لوجو يملأ العرض) محفوظة في scratchpad (`DisplayDashboard.design1.css/.jsx`) للرجوع إليها.**
...
| التاريخ | الوصف |
|---------|-------|
| 2026-07-01 | **🔀 تصميم بديل لشاشة العرض (كرت أفقي مستطيل) — 10 طلبات/صف.** حسب الطلب: كرت **أفقي** (`.disp-card { flex-direction: row }`) — **اللوجو يمين (حجم ثابت 96px) + الرقم شمال**، بادينج خفيف جداً (`0.4rem 0.5rem`) وفجوة صغيرة (`gap:0.45rem`) بين اللوجو والرقم فقط (مش لازق)، شبكة **`repeat(10, minmax(0,1fr))`** (كانت 12)، رقم `clamp(0.95rem–1.55rem)` يتكيّف بجانب اللوجو. **تعديل CSS فقط** (بلا مسّ للـ JSX — الـ RTL يضع اللوجو يميناً تلقائياً). **نسخة التصميم السابق (الرأسي 12/صف) محفوظة في scratchpad `DisplayDashboard.design1.css/.jsx` — يمكن الرجوع لها عند الطلب.** ✅ lint + build نظيف. |
