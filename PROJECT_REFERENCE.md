...
│   │   ├── LoadingScreen.css
│   │   ├── Toast.jsx           # تنبيه عائم غير معطِّل (بديل alert) + CSS — شاشة الفرع
│   │   ├── Toast.css
...
└── supabase/
    └── migrations/
        ...
        └── 032_foodics_background_sync.sql    # Optimistic UI: أعمدة retry + تحييد p_synced + fn_foodics_sync_* + جدولة pg_cron/pg_net للـ resync
```
...
foodics_delivery_status INTEGER                    -- آخر delivery_status من فوديكس (1,2,5,6) — وهو ما يرسله الـ sweeper بالضبط
synced_to_foodics       BOOLEAN DEFAULT false      -- هل آخر تحديث وصل فوديكس؟ (false = مهمة مزامنة معلقة؛ يرفعه service_role فقط منذ 032)
foodics_sync_attempts   SMALLINT DEFAULT 0         -- (032) عدد محاولات المزامنة الفاشلة (20 = توقف، 32767 = فشل دائم 404/422)
foodics_sync_next_at    TIMESTAMPTZ DEFAULT now()  -- (032) موعد المحاولة التالية (backoff تصاعدي 30s→8m)
...
| `rpc_kitchen_mark_ready_synced(p_session_id, p_order_internal_id, p_device_info, p_synced)` | زر "جاهز": preparing→ready + synced=false وتصفير عدادات المزامنة (p_synced مُتجاهَل منذ 032) | admin/user |
| `rpc_kitchen_mark_delivered(p_session_id, p_order_internal_id, p_device_info, p_synced)` | زر "تم التسليم": ready→completed + synced=false وتصفير العدادات (p_synced مُتجاهَل منذ 032) | admin/user |
...
| `fn_foodics_sync_mark_synced(p_order_id, p_delivery_status)` | (032) رفع synced_to_foodics بعد نجاح PUT — بحراسة تطابق الحالة المرسلة | service_role فقط |
| `fn_foodics_sync_fail(p_order_id, p_permanent)` | (032) تسجيل فشل محاولة مزامنة: عداد + backoff تصاعدي (سقف 8 دقائق)؛ permanent = إخراج من الطابور | service_role فقط |
...
[زر "جاهز" في المطبخ]  ← فوري منذ 2026-07-15 (Optimistic UI)
       → (0ms) الموديل يقفل والكارت ينتقل تفاؤلياً في الواجهة
       → (خلفية) Edge Function foodics-update-status (action: ready): RPC القاعدة أولاً
         (UPDATE status=ready, ready_at, foodics_delivery_status=2, synced=false + scan_log ready_scan) → رد فوري
       → (خلفية الخلفية — EdgeRuntime.waitUntil) PUT /orders/{id} {delivery_status: 2} → fn_foodics_sync_mark_synced
       ↓  (ينتقل لقسم "جاهز" في المطبخ + "جاهز للاستلام" عند العميل)
[زر "تم التسليم" في المطبخ]  ← نفس النمط
       → (0ms) تفاؤلي → (خلفية) RPC: UPDATE status=completed, delivered_at, foodics_delivery_status=5, synced=false + scan_log delivered
       → (waitUntil) PUT /orders/{id} {delivery_status: 5, delivered_at: <UTC المخزّن>}
       ↓
...
- **معالجة فشل فوديكس (منذ 032 — إعادة محاولة مضمونة):** المحلي يتحدث دائماً أولاً (`synced_to_foodics=false` في نفس الترانزاكشن). فشل الـ PUT → `fn_foodics_sync_fail` (عداد + backoff) → **sweeper**: pg_cron كل 30 ثانية (بحارس WHERE EXISTS) → pg_net → action `resync` في `foodics-update-status` (حتى 15 طلباً/دورة، يرسل آخر `foodics_delivery_status` مخزّن، يحترم 429، حد 20 محاولة ≈ ساعتان ونصف، 404/422 = فشل دائم). الصدى الوارد من فوديكس بعد نجاح PUT يؤكد المزامنة مجاناً في الـ webhook.
- **فشل تحديث القاعدة نفسها (شبكة المطبخ):** إعادة محاولة تلقائية واحدة من الواجهة، ثم رجوع الكارت تلقائياً لمكانه + Toast — فوديكس لا تُلمس قبل نجاح القاعدة، فلا تضارب ممكن.
...
- **Realtime Subscription** عبر `postgres_changes`:
  ...
  - `UPDATE` → يحدّث الطلب (ويُنهي القيد التفاؤلي إذا بلغت القاعدة الهدف)
  - `DELETE` → يزيل الطلب
- **طبقة التحديث التفاؤلي (2026-07-15):** خريطة pending في ref (orderId → target/patch/snapshot/timer):
  - `markOrderLocal(id, action)` — نقل الكارت فوراً (patch مطابق لما تكتبه الـ RPC) + snapshot للرجوع + مهلة أمان 15 ثانية (رجوع تلقائي بلا تأكيد)
  - `confirmOrderLocal(id)` — تثبيت بعد نجاح السيرفر (لا انتظار Realtime)
  - `revertOrderLocal(id)` — رجوع للحالة السابقة، ترجع false لو القيد مؤكد بالفعل (فلا تنبيه زائف)
  - `isOrderPending(id)` — لتعطيل زر الكارت المعلق مؤقتاً
  - `refetch` يعيد تطبيق القيود المعلقة فوق النتيجة (لا يرتد كارت تحرك للتو)
- يرجع: `{ orders, preparing, ready, delivered, newOrderFlag, refetch, markOrderLocal, confirmOrderLocal, revertOrderLocal, isOrderPending }`
...
- **Confirmation Modal (فوري منذ 2026-07-15):** التأكيد يقفل الموديل وينقل الكارت **في نفس اللحظة** (`markOrderLocal` — صفر شبكة في المسار الحرج، بلا حالة "جاري التنفيذ")، ثم يستدعي Edge Function `foodics-update-status` (action `ready`/`delivered`) في الخلفية مع **إعادة محاولة تلقائية واحدة** بعد 500ms (`invokeUpdateStatus`). النجاح → `confirmOrderLocal`؛ الفشل → `revertOrderLocal` (رجوع الكارت) + **Toast** غير معطِّل. الأزرار المعلقة تتعطل مؤقتاً (`isOrderPending`).
...
| التاريخ | الوصف |
|---------|-------|
| 2026-07-15 | **⚡ Optimistic UI + مزامنة فوديكس خلفية مع إعادة محاولة مضمونة (تسريع وقت الضغط).** زر "جاهز"/"تم التسليم"/سلة الحذف صار **فورياً**: الموديل يقفل والكارت ينتقل في نفس اللحظة (صفر شبكة في المسار الحرج)، القاعدة تتحدث في الخلفية عبر الـ Edge Function (داتابيز أولاً)، وفوديكس تتزامن في خلفية الخلفية. **Frontend:** `useOrders` أضاف طبقة تفاؤلية (`markOrderLocal`/`confirmOrderLocal`/`revertOrderLocal`/`isOrderPending` — خريطة pending بـ snapshot + مهلة أمان 15 ثانية + إعادة تطبيق فوق refetch/Realtime)؛ `Kitchen.jsx` يقفل الموديل فوراً ويزامن بالخلفية مع إعادة محاولة تلقائية واحدة، والفشل يرجع الكارت + **Toast** غير معطِّل (`Toast.jsx` جديد — بديل `alert`)؛ زر الكارت المعلق يتعطل مؤقتاً (منع أكشن فوق أكشن). **Edge Function `foodics-update-status`:** انقلب الترتيب — RPC القاعدة أولاً ثم الرد فوراً ثم PUT فوديكس عبر `EdgeRuntime.waitUntil` (يرسل دائماً آخر حالة مخزّنة `foodics_delivery_status` لا حالة الزر — يمنع سباق جاهز→تسليم، مع كشف ذاتي للكتابة الفائتة وإعادة فتح المزامنة)؛ + action **`resync`** سرّي (header `x-resync-secret` = env `FOODICS_RESYNC_SECRET`) يعالج حتى 15 طلباً معلقاً ويحترم 429. **Migration `032_foodics_background_sync.sql`:** عمودا `foodics_sync_attempts`/`foodics_sync_next_at` + فهرس جزئي؛ تحييد `p_synced` في RPCs المطبخ الثلاث (سد ثغرة تزييف علم المزامنة — رفعه صار حكراً على service_role) + تصفير العدادات؛ دالتا `fn_foodics_sync_mark_synced` (بحراسة الحالة) و`fn_foodics_sync_fail` (backoff تصاعدي 30s→8m، حد 20 محاولة، 404/422 = دائم)؛ جدولة **pg_cron كل 30 ثانية** (fallback دقيقة) → pg_net → resync، بحارس `WHERE EXISTS` فلا نداء بلا شغل معلق. **Webhook `foodics-webhook`:** الصدى المتطابق صار no-op (لا إعادة ختم `ready_at`/`completed_at` — كان يفسد `prep_duration_seconds`) ويُستغل كتأكيد مزامنة مجاني؛ forward-only صار صارماً (`<=` — يمنع completed↔cancelled)؛ الإلغاء المتأخر لا يمس طلباً مكتملاً (حماية سجل التسليم)؛ الطلبات/التحديثات الواردة من فوديكس تُعلَّم `synced_to_foodics=true` (لا PUT ارتدادي)؛ + صيغ أحداث الجمع `orders.created/updated` دفاعياً. **⚠️ نشر:** دليل كامل بأوامر جاهزة + اختبار قبول + rollback في **`docs/DEPLOY_032_BACKGROUND_SYNC.md`**. باختصار بالترتيب: (1) vault secrets لكل مشروع (`kz_project_url` + `kz_resync_secret`) و`npx supabase secrets set FOODICS_RESYNC_SECRET=...`؛ (2) migration 032 (يتطلب إضافتَي `pg_cron` + `pg_net` مفعّلتين)؛ (3) نشر `foodics-webhook` ثم `foodics-update-status` بـ `--no-verify-jwt` (إلزامية — بدونها الـ cron يُرفض 401)؛ (4) الواجهة. كل خطوة آمنة مستقلة — العميل القديم يعمل خلال 1-3. |
| 2026-07-12 | **📺 توجل إظهار/إخفاء هيدر شاشة العرض من شاشة الفرع (يتزامن realtime).** حسب الطلب: توجل جديد "الهيدر" (`.kt-header-toggle` — سويتش iOS) في هيدر شاشة الفرع بجوار dropdown وضع شاشة العرض: مفعّل (افتراضي) = هيدر شاشة العرض (الساعة/اسم الفرع/عداد الطلبات النشطة/زر الصوت) ظاهر؛ مطفأ = الهيدر مخفي بالكامل وتتمدد شبكة الطلبات مكانه. **Backend:** migration **`031_display_header_toggle.sql`** — عمود `show_header BOOLEAN DEFAULT true` على `branch_settings` + تحديث `rpc_branch_bootstrap` ليُرجعه ضمن `display_setting`. **الـ hook** `useBranchDisplaySetting` أضاف `showHeader` + `setShowHeader` (قراءة + bootstrap seed + realtime + upsert تفاؤلي — نفس نمط `displayMode`). **`DisplayDashboard`:** الهيدر ملفوف بـ `{showHeader && ...}`، و`useEffect` يضيف/يزيل class **`kz-display-header-hidden`** على `body` — قاعدة في `index.css` تخفي معها العناصر العائمة العامة: لوجو كبة زون (`.kz-fixed-logo`) + زر القائمة الجانبية (`.kz-sidebar-toggle` — class جديدة أُضيفت لزرّي التوجل في `UserSidebar`/`AdminSidebar`) ⇒ الشاشة تصير طلبات فقط. ملاحظة: إخفاء الهيدر يخفي معه زر الصوت — الصوت يظل شغالاً على حالته الأخيرة (افتراضي مفعّل). **⚠️ نشر:** طبّق `031` على قاعدتي التيست والبرود قبل نشر الواجهة (الواجهة تتحمّل غيابه — fallback إلى true — لكن الحفظ من التوجل يحتاج العمود). |
